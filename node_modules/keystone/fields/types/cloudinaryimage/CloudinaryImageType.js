...
cloudinaryimage.prototype.getRequestHandler = function(item, req, paths, callback) {
	...
	return function() {
		...
		if (req.body && req.body.action=='updateItem') {
			var x = 0;
			// saving from an image on user's pc
			for (var prop in req.files) {
				if (prop=='image_upload')
				{
					if (req.files.hasOwnProperty(prop))
					{
						fs.rename(req.files[prop].path, 'public/uploads/'+req.files[prop].name);
					}
				}
				x++;
			}
			// saving from a remote image
			if (x==0 && req.body.remote != '')
			{
				/////////////////////////
		...
		if (x!=0) { item.set(field.path, {url: 'http://localhost:3000/uploads/'+req.files[prop].name});
		item.save(); alreadycalledback = true; callback();}

		if (req.body.remote=='' && alreadycalledback==false)
		{ alreadycalledback = true; callback(); }
			...
			if (alreadycalledback == false) callback();
		}
